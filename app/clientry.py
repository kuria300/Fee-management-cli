...
from app.utils.file_reader import read_json, save_json, payment_path, user_path, course_path
...
def handle_auth():
    ...
    print("\n =====Available Commands:=====")
    print("register <username> <password> <student/admin>")
    print("login <username> <password>")
    print("exit")

    user_input= input('fee-management-cli$ ').strip().lower()

    if 'exit' in user_input:
        print('Exiting CLI...')
        exit()

    parser= argparse.ArgumentParser(prog='Fee management CLI', description="Fees management using the argparse CLI", epilog="python -m app")
...
def admin_menu():
    ...
    print("\n =====Admin Commands=====")
    print("add-course <name> <fees>")
    print("add_student_to_course <student_id> <course>")
    print("list_students_in_a_course")
    print("delete-course <course_name>")
    print("unenroll-student <student_id> <course_name>")
    print("logout")
    print("exit")

    user_input= input('(admin)fee-management-cli $ ').strip().lower()
    ...
    del_course_parser = subparsers.add_parser("delete-course", help="Delete a course by name")
    del_course_parser.add_argument("course_name")

    unenroll_parser = subparsers.add_parser("unenroll-student", help="Remove student from course")
    unenroll_parser.add_argument("student_id", type=int)
    unenroll_parser.add_argument("course_name")
        ...
        # admin
            ...
            for c in courses:
                if c['course_id'] == course_obj.course_id:
                    existing_ids = {s['id'] for s in c.get('students', [])}
                    for s in course_obj.students:
            ...
            if not current_user or current_user.role != "admin":
                print("Only admin can delete courses.")
                return

            courses= read_json(course_path)
            payments= read_json(payment_path)

            payments= [p for p in payments if p['course_name'] != args.course_name]
            course_to_del= next((c for c in courses if c['course_name'] == args.course_name), None)
            ...
            save_json(course_path, courses)
            save_json(payment_path, payments)

            print(f'Course : {args.course_name} and related payments deleted successfully.')

        elif args.command == 'unenroll-student':
            if not current_user or current_user.role != "admin":
                print("Only admin can unenroll students.")
                return
            
            courses= read_json(course_path)

            course= next((c for c in courses if c['course_name'] == args.course_name), None)
            ...
            updated_students = [s for s in course["students"] if s["id"] != args.student_id]

            if len(updated_students) == len(course["students"]):
                print('student not enrolled in this course')
                return

            course["students"]= updated_students

            save_json(course_path, courses)

            print(f'student unenrolled successfully from {args.course_name}')
    except SystemExit:
        print('Invalid Command')
...
def stud_menu():
    ...
    print("\n ======Student Commands===== ")
    print("add-payment <course_name> <amount>")
    print("logout")
    print("exit")

    user_input= input('(student)fee-management-cli $ ').strip().lower()
...
def main():
    ...
    print(" --Welcome to the Fee Management CLI-- ")
